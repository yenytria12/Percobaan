...
if (isset($_POST["submit"])) {
    ...
    if (!is_numeric($gpa) or ($gpa <= 0)) {
        $error_message .= "-IPKharusdiisidenganangka";
    }
    if ($error_message === "") {
        $nim = mysqli_real_escape_string($connection, $nim);
        ...
        $birth_month = mysqli_real_escape_string(
            $connection,
            $birth_month
        );
...
<!DOCTYPEhtml>
    ...
    <head>
        <metacharset="UTF-8">
            ...
            <linkhref="assets /style.css" rel="stylesheet">
    </head>
    ...
    <body>
        <divclass="container">
            <divid="header">
                ...
                <h2>TambahDataMahasiswa</h2>
                <?php
                if ($error_message !== "") {
                    echo "<divclass='error'>$error_message</div>";
                }
                ?>
                <formid="form_mahasiswa" action="student_add.php" method="post">
                    <fieldset>
                        <legend>MahasiswaBaru</legend>
                        <p>
                            <label for="nim">NIM: </label>
                                <input type="text"name="nim"id="nim"value="<?php echo
                                                                            $nim ?>"placeholder="Contoh:12345678">(8 digitangka)
                        </p>
                        <p>
                        ...
                        <p>
                            <label for="birth_city">TempatLahir: </label>
                            ...
                            <label for="birth_date">Tanggal Lahir: </label>
                                <select name="birth_date" id="birth_date">
                                    ...
                                    <select name="birth_month">
                                        <?php
                                        foreach ($arr_monthas as $key => $value) {
                                            if ($key == $birth_month) {
                        ...
                        <p>
                            <labelfor="gpa">IPK: </label>
                                <inputtype="text"name="gpa"id="gpa"value="<?php echo
                                                                            $gpa ?>"placeholder="Contoh:2.75"> (angkadesimaldipisah dengan
                                    karaktertitik".")
                        </p>
                    </fieldset>
                    <br>
